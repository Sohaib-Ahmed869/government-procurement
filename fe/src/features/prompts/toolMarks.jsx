...
export const MARK_BY_VALUE = MARKS;

export function toolLabel(value) {
  return MARKS[value]?.label || value;
}

/* One mark, at one size.

   `size` is the box every mark shares. The per-mark `scale` is applied inside
...
export default function ToolMark({ tool, size = 16 }) {
  ...
  return (
    <svg
      ...
      focusable="false"
    >
      <g transform={`translate(12 12) scale(${mark.scale}) translate(-12 -12)`}>
        <path d={mark.path} />
      </g>
    </svg>
  );
